...
- Colab instability (frequent disconnects and session limits) meant the free
  ...
  end-to-end run (10 epochs, 320px). A longer run (50 epochs, 640px) was
  attempted afterward but never completed — Colab's runtime kept disconnecting
  or resetting before it could finish, and there was no time left to keep
  retrying. The 10-epoch/320px run ended up being the final, submitted result.
- That run's metrics and failure cases (see README Sections 6–7) are reported
  honestly rather than omitted or dressed up, including where the model is
  clearly weak (e.g. the violation classes) — this is the real result, not a
